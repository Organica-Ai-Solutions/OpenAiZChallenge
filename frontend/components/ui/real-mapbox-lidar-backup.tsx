          ...
          <div className="bg-slate-800/50 rounded-lg p-4 border border-slate-600">
            ...
            <div className="grid grid-cols-5 gap-2 mb-3">
              {[1, 2, 3, 4, 5].map((zoom) => (
                <Button
                  key={zoom}
                  size="sm"
                  variant={lidarVisualization.hdZoomLevel === zoom ? "default" : "outline"}
                  className={`text-xs ${
                    lidarVisualization.hdZoomLevel === zoom 
                      ? "bg-cyan-600 hover:bg-cyan-700 text-white" 
                      : "border-slate-600 text-slate-300 hover:bg-slate-700"
                  }`}
                  onClick={() => {
                    console.log(`🔍 Setting HD zoom to ${zoom}m`)
                    
                    // Update zoom level (this would need to be passed from parent)
                    if (typeof lidarVisualization.setHdZoomLevel === 'function') {
                      lidarVisualization.setHdZoomLevel(zoom)
                    }
                    ...
                    if (map.current && mapLoaded) {
                      ...
                      fetch('/api/lidar/hd', {
                        ...
                        body: JSON.stringify({
                          ...
                          lon: lng,
                          zoom_meters: zoom,
                          archaeological_context: 'amazon_settlement'
                        })
                      })
                      ...
                      .then(data => {
                        if (data.status === 'success') {
                          ...
                          if (data.archaeological_analysis?.features_detected) {
                            // Add HD features to map
                            console.log('🏛️ Adding HD features to map:', data.archaeological_analysis.features_detected.length)
                          }
                        } else {
            ...
            <div className="text-xs text-slate-400 space-y-1">
              <div className="flex justify-between">
                ...
                <span>Micro-features, post holes</span>
              </div>
              <div className="flex justify-between">
                <span>3m: High Detail</span>
                <span>Foundations, structures</span>
              </div>
              <div className="flex justify-between">
                <span>4-5m: Standard Detail</span>
